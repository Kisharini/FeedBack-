...
function getAuthHeaders() {
  ...
  if (rawToken?.startsWith("{")) {
    try {
      ...
      rawToken = parsed.token || parsed.accessToken;
    } catch (e) {
...
function ListingCard({ listing, onEdit, onDelete, onToggleArchive, onRelist }) {
  // FIX #3: Use uppercase status strings to match API enums
  const isClaimed = listing.status === "CLAIMED";

  // FIX #4: Derive archived from status instead of undefined listing.archived
  const archived = listing.status === "ARCHIVED";

  // Derive expiry label and warning from API data
  const expiryWarning = listing.status === "EXPIRING";
  const expiryLabel =
    isClaimed
      ? "Pickup Completed"
      : archived
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div
            style={{
              ...
              alignItems: "center",
              gap: 4,
              color: expiryWarning ? C.error : C.onSurfaceVariant,
            }}
          >
            <span
              className="material-symbols-outlined"
            ...
            <span style={{ fontSize: 12, fontWeight: 700 }}>{expiryLabel}</span>
          </div>
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <span style={{ fontSize: 12, fontWeight: 700 }}>
              {listing.claimedCount ?? listing.claimed ?? 0}/{listing.quantity ?? listing.qty} Claimed
            </span>
          </div>
        </div>
        ...
        >
          {/* FIX #5: Show edit/delete only for non-claimed, non-archived listings */}
          {!isClaimed && !archived ? (
            <>
              <ActionBtn
                icon="edit"
              ...
              <div
                style={{
                  width: 1,
                  background: `${C.outlineVariant}33`,
                  alignSelf: "center",
                  height: 32,
                }}
              />
            ...
            <>
              <ActionBtn
                icon={archived ? "unarchive" : "inventory"}
...
export default function FeedbackPro() {
  ...
  useEffect(() => {
    async function fetchListings() {
      try {
        ...
        if (!response.ok) throw new Error("Failed to load records from database");
  ...
  function openEdit(listing) {
    ...
    const expiryDateObj = new Date(listing.expiryAt);
    const dateStr = expiryDateObj.toISOString().split("T")[0];
    const timeStr = expiryDateObj.toTimeString().split(" ")[0].substring(0, 5);
    ...
    setForm({
      ...
      unitPrice: listing.unitPrice ? String(listing.unitPrice) : "",
      date: dateStr,
      time: timeStr,
  ...
  async function handleSubmit() {
    ...
    if (
      ...
      (!form.unitPrice || isNaN(parseInt(form.unitPrice, 10)))
    ) {
      alert("Please enter a valid unit price");
      return;
    }

    const expiryAtISO = new Date(`${form.date}T${form.time}:00`).toISOString();
    ...
    try {
      if (editingListing) {
        ...
        if (!response.ok)
          throw new Error("Could not store new listing in backend");
        const savedItem = await response.json();
        setListings((ls) => [savedItem, ...ls]);
      }
  ...
  async function handleToggleArchive(id) {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to update status record");
      const updatedItem = await response.json();
      setListings((ls) => ls.map((l) => (l.id === id ? updatedItem : l)));
    } catch (err) {
      alert(err.message);
    }
  }
  ...
  async function handleRelist(id) {
    try {
      ...
      const response = await fetch(`/api/listings/${id}`, {
        ...
        body: JSON.stringify({
          status: "AVAILABLE",
          expiryAt: freshExpiry.toISOString(),
        }),
      });

      if (!response.ok) throw new Error("Failed to relist listing.");
      const updatedItem = await response.json();
    ...
    >
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              fontSize: 14,
              fontWeight: 600,
              boxShadow: "0 2px 6px rgba(70,132,50,0.2)",
          ...
          <div>
            ...
            >
              <div>
                ...
                >
                  Oversee your surplus food distribution and track your
                  community impact.
                </p>
              </div>
            </header>
      ...
      <button
        ...
        style={{
          ...
          color: C.onPrimaryContainer,
          border: "none",
          boxShadow: "0 4px 16px rgba(70,132,50,0.3)",
          cursor: "pointer",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
      ...
      <Modal open={showListingModal} onClose={() => setShowListingModal(false)}>
        ...
        >
          ...
          >
            <FormInput label="Listing Type" id="f-type">
              <select
                id="f-type"
                value={form.type}
                onChange={(e) =>
          ...
          {form.type === "DISCOUNTED" && (
            <FormInput label="Unit Price (RM)" id="f-price">
              <input
                id="f-price"
          ...
          <FormInput label="Image URL" id="f-image">
            <input
              id="f-image"
              value={form.imageUrl || ""}
              onChange={(e) =>
          ...
          <FormInput label="Pickup Location" id="f-location">
            <input
              id="f-location"
              value={form.location}
              onChange={(e) =>
          ...
          <FormInput
            label="Pickup Instructions & Description"
            id="f-description"
          >
            <textarea
              id="f-description"
              ...
              placeholder="Where and when to pick up, or item descriptions..."
              rows={3}
        ...
        </div>{/* ← FIX #1: This closing div was missing in the original */}
      </Modal>
